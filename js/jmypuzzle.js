...
$.fn.jMyPuzzle = function(o) {  
	...
	return this.each(function() { 
			...
			$('.reset').click(function(){							// on reset event
				...
				for(i=0; i<nbElts; i++){
					if(elts[i].initialN != i){						// if something is wrong in the order
						for(j=i;j<nbElts;j++){						// look for the missing piece
					...
					if(elts[n].initialN + 1 == answerTab[n]){ 
		    			...
		    			for(var i=0; i < answerTab.length; i++){
				    			if(elts[n+i] && answerTab[i+j] == elts[n+i].initialN + 1){
				    				if(i>0){
				    					elts[n+i].animate({'top': (posTop +7) + "px"}, 75).addClass(o.classOnMiValid).animate({'top': posTop + "px"}, 100);
				    				}
				    				else{
		...
		function onDrag(e, ui, elt, elts){
			...
			if(oldPos && this.eltPos.left == oldPos.left) { return; }							// Not moving = doing nothing
			move = (oldPos && this.eltPos.left > oldPos.left ? forward : backward);			// check whether the move is forward or backward
